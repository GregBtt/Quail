...
#+:use-dclm(declaim (sb-ext:maybe-inline active-status-p)) ;25NOV2024
(defun active-status-p (s)
  "Checks for active case status "
  (eq s t))

#+:use-dclm(declaim (sb-ext:maybe-inline invalid-status-p)) ;25NOV2024
(defun invalid-status-p (s)
  "Checks for invalid case status "
  (or (eq s :invalid)  (eq s :invalid-ignore)))

#+:use-dclm(declaim (sb-ext:maybe-inline ignore-status-p)) ;25NOV2024
